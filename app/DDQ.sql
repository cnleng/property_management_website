...
CREATE TABLE owner (Owner_ID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, Number_Of_Properties INT NOT NULL, Name VARCHAR(50) NOT NULL);

DROP TABLE IF EXISTS lease;
CREATE TABLE lease (Lease_ID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, End_Date DATE NOT NULL, Length_Of_Lease INT NOT NULL, Rent DECIMAL (19,2) NOT NULL); 

DROP TABLE IF EXISTS property;
CREATE TABLE property (Property_ID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, Street_Address VARCHAR(100) NOT NULL, Unit_Number VARCHAR(20), Zip_Code VARCHAR(10) NOT NULL, Square_Feet DECIMAL (19,2) NOT NULL, Rooms INT NOT NULL, Bathrooms DECIMAL (5,1) NOT NULL, Management_Fee DECIMAL (19,2) NOT NULL, Lease_ID INT NOT NULL, FOREIGN KEY (Lease_ID) REFERENCES lease(Lease_ID) ON DELETE CASCADE);

DROP TABLE IF EXISTS tenant;
CREATE TABLE tenant (Tenant_ID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, Property_ID INT NOT NULL, Lease_ID INT NOT NULL, Name VARCHAR(50) NOT NULL, Credit_Score INT, Social_Security INT NOT NULL, Date_Of_Birth DATE NOT NULL, FOREIGN KEY (Property_ID) REFERENCES property (Property_ID) ON DELETE CASCADE, FOREIGN KEY (Lease_ID) REFERENCES lease (Lease_ID) ON DELETE CASCADE);

DROP TABLE IF EXISTS lease_owner;
CREATE TABLE lease_owner (lID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, Owner_ID INT NOT NULL, Lease_ID INT NOT NULL, FOREIGN KEY (Owner_ID) REFERENCES owner (Owner_ID), FOREIGN KEY (Lease_ID) REFERENCES lease (Lease_ID) ON DELETE CASCADE);

DROP TABLE IF EXISTS property_owner;
CREATE TABLE property_owner (pID INT AUTO_INCREMENT PRIMARY KEY NOT NULL, Property_ID INT NOT NULL, Owner_ID INT NOT NULL, FOREIGN KEY (Property_ID) REFERENCES property (Property_ID) ON DELETE CASCADE, FOREIGN KEY (Owner_ID) REFERENCES owner (Owner_ID) ON DELETE CASCADE);
